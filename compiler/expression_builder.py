...
def expr_to_masm(expr: mast.Expression, ns: Namespace) -> list[str]:
	...
	if expr.operator.value == '=':
		...
		if isinstance(expr.right, mast.Identifier):
			output.append(f'MOV {ns[expr.left.value].addr_as_str} {ns[expr.right.value].addr_as_str}')
			...
			output.append(f'MOV {ns[expr.left.value].addr_as_str} {ns.addr_as_str(free_addrs[0])}')
	...
	if isinstance(expr.right, mast.Identifier):
		...
		output.append(f'{addr_instr} {free_addrs[0]} {ns[expr.right.value].addr_as_str}')
